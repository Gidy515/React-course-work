...
const books = getBooks(); // this books variable grabs the whole array of books

// Destructuring Arrays
const book = getBook(1); // this book variable grabs the whole object of the book with id 2, which is "The Cyberiad"
...
const booksAfterUpdate = booksAfterDelete.map(
  (book) => (book.id === 1 ? { ...books, pages: 1210 } : book) // this line says that if the book id is 1, then return an empty object, otherwise return the book object as it is. This way we are updating the book with id 1 to an empty object, which means we are deleting all the properties of that book and making it an empty object.
);
