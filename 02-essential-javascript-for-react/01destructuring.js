...
const [primaryGenre, secondaryGenre] = genres; // this is the destructuring assignment syntax for arrays, it allows us to extract multiple values from an array and assign them to variables in a single statement. In this case, we are extracting the first two values from the genres array and assigning them to variables called primaryGenre and secondaryGenre. it makes the extraction by the order of the values in the array, so primaryGenre will be assigned the value of the first element in the genres array, which is "science fiction", and secondaryGenre will be assigned the value of the second element in the genres array, which is "humor".
